...
public:
    ...
    friend Iterator operator+(const Iterator<T>& lit, const Iterator<T>& rit) {
        Iterator<T> result(lit); // создаем копию левого итератора
        while (result.current != rit.current) { // пока не дойдем до правого итератора
            ++result; // переходим к следующему элементу списка
        }
        return result; // возвращаем новый итератор
    }
    Iterator& operator++() { //++i
    ...
    Iterator& operator=(const Iterator& other) {
        if (this != &other) { // Если переданный итератор не равен текущему
